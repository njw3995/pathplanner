...
BATCH_DIALOG = r'''import 'dart:convert';
import 'dart:io';
...
def find_repo(start: Path) -> Path:
    ...
    for candidate in candidates:
        if (candidate / 'lib').is_dir() and (candidate / 'pubspec.yaml').is_file():
            return candidate
        nested = candidate / 'pathplanner'
        if (nested / 'lib').is_dir() and (nested / 'pubspec.yaml').is_file():
            return nested
    raise FileNotFoundError('Could not find PathPlanner repo root. Run from the repo root or pass --repo.')


def backup(path: Path) -> None:
    backup_path = path.with_suffix(path.suffix + '.gif_asset_fix.bak')
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        print(f'  skipped {label}: pattern not found')
        return text
    return text.replace(old, new, 1)


def patch_trajectory_render_dialog(repo: Path) -> None:
    path = repo / 'lib' / 'widgets' / 'dialogs' / 'trajectory_render_dialog.dart'
    if not path.is_file():
        print('trajectory_render_dialog.dart not found, skipping')
        return
    backup(path)
    ...
    text = replace_once(
        ...
        'null-safe individual GIF frame capture',
    )
    ...
    text = replace_once(
        ...
        """      img.PngDecoder decoder = img.PngDecoder();\n      img.GifEncoder encoder = img.GifEncoder(\n        numColors: 64,\n        dither: img.DitherKernel.none,\n      );\n      var addedFrames = 0;\n      final denom = message.length <= 1 ? 1 : message.length - 1;\n\n      for (int i = 0; i < message.length; i++) {\n        try {\n          final decoded = decoder.decode(message[i]);\n          if (decoded != null) {\n            encoder.addFrame(decoded, duration: 4);\n            addedFrames++;\n          }\n        } catch (_) {\n          // Skip bad captures instead of crashing the desktop app.\n        }\n        controller.sendResult((\n          progress: (i / denom),\n          bytes: null, // Only bother sending the bytes for the final result\n        ));\n      }\n\n      if (addedFrames == 0) {\n        return (progress: 1.0, bytes: null);\n      }\n\n      return (progress: 1.0, bytes: encoder.finish());\n""",
        'null-safe individual GIF encoding',
    )
...
def patch_batch_dialog(repo: Path) -> None:
    path = repo / 'lib' / 'widgets' / 'dialogs' / 'batch_path_export_dialog.dart'
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        backup(path)
    path.write_text(BATCH_DIALOG, encoding='utf-8')
    print(f'patched {path.relative_to(repo)}')


def main() -> int:
    parser = argparse.ArgumentParser(description='Repair PathPlanner batch exports and GIF null decoding.')
    parser.add_argument('--repo', default='.', help='PathPlanner repo root')
    args = parser.parse_args()

    repo = find_repo(Path(args.repo).resolve())
    print(f'PathPlanner repo: {repo}')
    patch_trajectory_render_dialog(repo)
    patch_batch_dialog(repo)
    print('Done. Run: flutter analyze')
    return 0
